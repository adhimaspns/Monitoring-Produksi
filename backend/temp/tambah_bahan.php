<?php
  ...
  if (!empty($_GET['ket']) && $_GET['ket'] == 'bhn_baku' ) {
    ...
    if ($jumlah_qty == 0) {
      echo "<script>alert('Masukkan jumlah yang sesuai!');window.location.href='../../frontend/produksi/produksi.php?id_produksi=$id_produksi&page=produksi'</script>";
    } else {
      ...
      if ($cekId != 0) {
        ...
        $nama_produk = $data_pra['nama_produk'];
        ...
        //! UPDATE TEMP 
        $sqlSub          = mysqli_query($host, "UPDATE temp SET sub_total = '$subtotal' WHERE  bhn_id = '$bhn_id' AND produksi_id = '$id_produksi'  ");
